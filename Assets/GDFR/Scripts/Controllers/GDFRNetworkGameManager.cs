...
namespace GDFR
{
    public class GDFRNetworkGameManager : MonoBehaviour
    {
        ...
        public void SetupBaseMessageHandlers()
        {
            ...
            GDFRNetworkManager.Instance.localClient.RegisterHandler(MsgIndexes.GroupedDrawMessage,
                NetOnGroupDrawMessage);
            GDFRNetworkManager.Instance.localClient.RegisterHandler(MsgIndexes.InitiativeSelected,
                NetOnRecieveInitiative);
        ...
        {
            return Toolbox.Instance.playerProfiles[mController.currentPlayer].networkProfile.networkId ==
                   GDFRNetworkManager.Instance.LocalConnectionId;
        }
        ...
        {
            Debug.Log("NETWORK: " + " Player " + mController.currentPlayer + "- Position: " +
                      mController.PlayersPosition[mController.currentPlayer] + " - State: DrawPhase1");
            ...
            if (GDFRNetworkManager.Instance.IsLocalClientTheHost())
            {
                ...
                GroupedDrawMessage outMess =
                    new GroupedDrawMessage(GroupDrawPhase.Phase1, mController.starDeck.Id, cards, toDecks);
        ...
        public IEnumerator State_Network_DrawPhase2()
        {
            ...
            if (GDFRNetworkManager.Instance.IsLocalClientTheHost())
            {
                ...
                foreach (Deck pDeck in mController.playerDecks)
                {
                    ...
                    if (pDeck.enabled)
                    {
                        Card secondCard = mController.mainDeck.DrawRandomCardOfSymbolGroup(
        ...
        public IEnumerator State_Network_DrawPhase3()
        {
            ...
            if (GDFRNetworkManager.Instance.IsLocalClientTheHost())
            {
                ...
                GroupedDrawMessage outMess =
                    new GroupedDrawMessage(GroupDrawPhase.Phase3, mController.mainDeck.Id, cards, toDecks);
            ...
            for (int i = 0; i < phase3DrawData.cardIds.Length; i++)
            {
                ...
                c.CurrentRace = Toolbox.Instance.gameSettings.RulesVariant == GameSettings.RulesVariantType.GoblinsRule
                    ? Race.Goblin
                    : Race.Fairy;
        ...
        public IEnumerator State_Network_Initiative()
        {
            if (GDFRNetworkManager.Instance.IsLocalClientTheHost())
            {
                    ...
                    while (!foundPlayer)
                    {
                        ...
                        for (int idx = 0; idx < Toolbox.Instance.gameSettings.numberOfPlayers; idx++)
                        {
                                ...
                                (Random.Range(0, 2) == 0))
                            {
                                ...
                                GDFRNetworkManager.Instance.TriggerEventIfHost(MsgIndexes.InitiativeSelected,
                                    new IntMessage(idx));
                                break;
                            }
                ...
                {
                    GDFRNetworkManager.Instance.TriggerEventIfHost(MsgIndexes.InitiativeSelected,
                        new IntMessage(Random.Range(0, Toolbox.Instance.gameSettings.numberOfPlayers)));
                }
            }
            ...
            mController.avatars[mController.PlayersPosition[mController.currentPlayer]].avatarGlowSprite.gameObject
                .SetActive(true);
        }
        ...
        private void NetOnCardPlayed(NetworkMessage message)
        {
            ...
            Card card = mController.playerDecks[mController.PlayersPosition[mController.currentPlayer]]
                .GetExactCard(mess.cardPlayed);
