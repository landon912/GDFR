    ...
    public abstract class AIModule
    {
        ...
        private int CalculateDirectEffects(Card[] centerCards, Card cardBeingPlayed, List<Card> allPlayerCards)
        {
            ...
            foreach (Card centerCard in centerCards)
            {
                ...
                bool flipped = centerCard.CurrentRhyme == cardBeingPlayed.CurrentRhyme ||
        ...
        private int CalculatePassiveEffects(Card[] centerCards, Card cardBeingPlayed, List<Card> allPlayerCards)
        {
            ...
            foreach (Card playerCard in allPlayerCards)
            {
                ...
                foreach (Card centerCard in centerCards)
                {
                    ...
                    if (wouldFlip)
                    {
                        ...
                        if (!playerCard.AreStarsShowing && playerCard.CurrentSymbol == centerCard.CurrentSymbol)
                        {
                            ...
                            {
                                Debug.Log(centerCard + " is currently of use to player before flip when compared to " +
                                          playerCard + "; taking away one aid point");
                                aidToPlayers--;
                            }
                            ...
                            {
                                Debug.Log(centerCard +
                                          " is currently bad for the player before flip when compared to " +
                                          playerCard + "; adding one aid point");
                                aidToPlayers++;
                            }
                        }
                        ...
                        if ((playerCard.AreStarsShowing || centerCard.CurrentRhyme == playerCard.CurrentRhyme) &&
                            playerCard.CurrentSymbolGroup == centerCard.CurrentSymbolGroup &&
                            playerCard.CurrentSymbol != centerCard.CurrentSymbol)
                        {
                            if (playerCard.CurrentRace == Race.Goblin && centerCard.CurrentRace == Race.Goblin)
                            {
                                Debug.Log(centerCard + " is currently of use to player before flip when compared to " +
                        ...
                        {
                            //would take
                            if (playerCard.CurrentSymbolGroup == centerCard.CurrentSymbolGroup &&
                                playerCard.CurrentSymbol != centerSymbol)
                            {
                                //since player card is a star card or rhyme
                                if (playerCard.CurrentRace == Race.Goblin && centerRace == Race.Goblin)
                                {
                                    Debug.Log(centerCard +
                                              " would be good for the player after flip when compared to " +
                                              playerCard + "; taking away one aid point");
                                    aidToPlayers++;
                                }
                                ...
                                {
                                    Debug.Log(centerCard + " would be bad for the player after flip when compared to " +
                                              playerCard + "; adding one aid point");
                                    aidToPlayers--;
                                }
                            }
                        ...
                        else if (playerCard.CurrentSymbol == centerSymbol)
                        {
                            ...
                            if (playerCard.CurrentRace == Race.Goblin && centerRace == Race.Fairy)
                            {
                                Debug.Log(centerCard + " would be good for the player after flip when compared to " +
                                          playerCard + "; adding one aid point");
                                aidToPlayers++;
                            }
                            ...
                            {
                                Debug.Log(centerCard + " would be bad for the player after flip when compared to " +
                                          playerCard + "; taking one aid point");
                                aidToPlayers--;
                            }
                        }
                    }
                ...
                CheckAndFlipCardIfNeeded(cardBeingPlayed, playerCard, out cardAddedToCenterRace,
                    out cardAddedToCenterSymbol);
                ...
                if (cardAddedToCenterRace == Race.Goblin)
                {
                    ...
                    {
                        aidToPlayers--;
                        Debug.Log("Would add a goblin of symbol " + cardAddedToCenterSymbol +
                ...
                else
                {
                    ...
                    {
                        aidToPlayers++;
                        Debug.Log("Would add a fairy of symbol " + cardAddedToCenterSymbol +
        ...
        private int GoblinsRuleCalculateDirectEffects(Card[] centerCards, Card cardBeingPlayed,
        ...
        private int GoblinsRuleCalculatePassiveEffects(Card[] centerCards, Card cardBeingPlayed,
            List<Card> allPlayerCards)
        {
            int aidToPlayers = 0;
